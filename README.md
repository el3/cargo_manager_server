...
1: Push to the bare repository: `git push production_server_user@production_server_ip:cargo_manager_server cargo_manager` `cargo_manager` being the branch in this case.
...
1: Go to the temprepo folder: `cd /tmp/temprepo/`
1: Copy a couple of files to the webapp folder:
  ```plaintext
...
1: Go to the webapp folder: `cd /var/www/cargo_manager_server/`
1: Change the permission an ownership of the folder: `sudo chown -R production_server_user: . && chmod 400 .env`
1: Start the service: `docker compose up --renew-anon-volumes --no-deps --detach`
1: Upgrade the db in a temporary container: `echo 'flask db upgrade' | docker compose run --rm backend sh`
